        ...
        switch (command) {
            ...
            case 'define':
                ...
                try {
                    ...
                    const reply = `
*🔠 Word:* ${q}
*📖 Definition:* ${rayyy.data.list[0].definition.replace(/\[/g, '').replace(/\]/g, '')}
*💭 Example:* ${rayyy.data.list[0].example.replace(/\[/g, '').replace(/\]/g, '')}
               `
            ...
            case 'song':
                ...
                const downloadSong = async (randomName, query) => {
                    try {
                        ...
                        songName = songName = songName = songName.replace(/\?|<|>|\*|"|:|\||\/|\\/g, '') //removing special characters which are not allowed in file name
                        // console.log(link);
        ...
        headerType: 2 */
            ...
            case 'add':
                {
                    ...
                    if (users.length == 0)
                        return replay(`Please write the number of the person you want to add to thhis group`)
                    await bot
                        .groupParticipantsUpdate(from, [users], 'add')
            ...
            case 'speak':
                {
                    ...
                    let texttosay = text ? text : m.quoted && m.quoted.text ? m.quoted.text : m.text
                    ...
                    bot.sendMessage(
                        from,
                        { audio: { url: texttospeechurl }, mimetype: 'audio/mpeg', fileName: `botSpeechEngine.mp3` },
                        { quoted: m }
                    )
                }
                break
          ...
          headerType: 2  */
            ...
            case 'manga':
                ...
                bot.sendMessage(
                    from,
                    { image: { url: srh.data[0].images.jpg.large_image_url }, caption: mang },
                    { quoted: m }
                )
                break
  ...
  {buttonId: `${prefix}neko`, buttonText: {displayText: `>>`}, type: 1},
      ...
      headerType: 2  */
            ...
            case 'help':
                ...
                bot.sendMessage(
                    from,
                    { video: { url: botselectionn }, gifPlayback: true, caption: txtt },
                    { quoted: m }
                )
                break
                break
            ...
            case 'element':
                ...
                if (search === undefined)
                    return reply(
                        `Please provide me a valid element by visiting here !\n\nhttps://en.m.wikipedia.org/wiki/Periodic_table`
                    )
            ...
            case 'screenshot':
                ...
                try {
                    ...
                    await bot.sendMessage(
                        from,
                        { image: resImage, caption: `_Here's how this url looks like:_\n${args[0]}\n` },
                        { quoted: m }
                    )
                } catch (error) {
                    reply(
...
let buttons = [
        ...
        */
            ...
            case 'search':
                {
                    if (!args[0])
                        return reply(`Example: ${prefix + command} <query>\nUses : ${prefix + command} anything...`)
            ...
            case 'gify':
                if (!ter) return reply(' No query provided!')
                try {
                    let { data: gi } = await axios.get(
                        `https://g.tenor.com/v1/search?q=${ter}&key=LIVDSRZULELA&limit=8`
                    )
            ...
            case 'del':
                {
...

                    {
                        ...
                        if (
                            ...
                            (quotedMessage.conversation && quotedMessage.conversation.includes('📰')) ||
                            (quotedMessage.conversation && quotedMessage.conversation.includes('Rank'))
                        ) {
            ...
            case 's':
                {
                    ...
                    if (quoted.seconds > 10) return reply('Please send video less than 10 seconds')
                    const pack = text
                        .replace(/--\w+(\s|$)/g, '')
                        .replace(/--\w+/, '')
                        .split('|')
            ...
            case 'add':
                {
                    ...
                    await bot
                        .groupParticipantsUpdate(from, [users], 'add')
                        .then((res) => reply(`User Added Successfully!`))
                        .catch((err) => reply(`Cannot add that user to this group!`))
                }
                break
            ...
            case 'bc':
                {
                    ...
                    for (let i of anu) {
                        ...
                        await bot.sendMessage(i, {
                            video: { url: 'https://telegra.ph/file/3c3f94c8463e7f9c29d73.mp4' },
                            mimetype: 'video/mp4',
                            fileName: `bc.mp4`,
                            caption: `${txt}\n\n🍀 *Author:* ${pushname}`
                        })
            ...
            case 'chat':
                {
                    ...
                    await axios
                        .get(`https://api.simsimi.net/v2/?text=${q}&lc=en`)
                        .then((response) => {
            ...
            case 'anime':
                {
                    ...
                    if (result.trailer.url !== null) details += `*Trailer:* ${result.trailer.url}\n\n`
                    details += `*URL:* ${result.url}\n\n`
                    if (result.background !== null) details += `*Background:* ${result.background}\n\n`
                    details += `*Description:* ${result.synopsis.replace(/\[Written by MAL Rewrite]/g, '')}`
                    bot.sendMessage(
                        from,
      ...
      headerType: 4 */
        ...
        headerType: 4 */
            ...
            case 'neko2':
                ...
                var wbutsss = [{ buttonId: `${prefix}neko2`, buttonText: { displayText: `>>` }, type: 1 }]
                let buttonssMessage = {
            ...
            case 'snk':
            default:
                if (isCmd) await bot.sendMessage(from, { text: `Command Rejected:- \n\n❌That is an invalid Command!` })
        }
    } catch (e) {
        console.log(e)
    }
